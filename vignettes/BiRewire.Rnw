...
\item 	generate rewired versions of a DSG;
\item 	generate a set of netwoks correctly drawn from the suitable null-model starting from the initial DSG;
\end{enumerate}
...
BiRewire requires the R packages {\bf igraph} and {\bf{slam}} (see \cite{igraph}, \cite{slam} ) available at the CRAN repository.
...
The function \textbf{birewire.analysis} returns the Jaccard similarity sampled every \textit{step} SSs (in the example above step is equal to 5000). In the resulting plots the value of the analyitically derived lower bound to the number of switching steps is also visualised {\bf \$N }. For more details see the the documentation.\\ 
...
This function uses the SA to generate a set of $K$ bipartite networks drawn from the null model given by an initial bipartite graph. The function creates a main folder (\textit{path} input parameter) and a set of subfolders in order to have maximum 1000 files per folder.
