...
Similar procedures have been implemented in order to manage undirected networks.
...
\item 	perform the same analysis described in point 1,2,3,5 and 6 for undirected graphs.
\end{enumerate}
...
BiRewire requires the R packages {\bf igraph}, {\bf{slam}} and {\bf tsne} (see \cite{igraph}, \cite{slam}, \cite{tsne} ) available at the CRAN repository.
...
<<Projections>>=
#use a smaller graph!
gg <- bipartite.random.game(n1=10,n2=4,p=0.5)
...
#birewire.visual.monitoring.bipartite(ggg,display=T,n.networks=75,sequence=c(1,10,200,500,"n",10000),ncol=3)
#Now we can generate a null model
#birewire.sampler.bipartite(ggg,K=10000,path="/home/andrea/example")
@
...
\bibitem{Iorio} Iorio, F. and Gobbi, A. (2015) \emph{In preparation} In preparation.
